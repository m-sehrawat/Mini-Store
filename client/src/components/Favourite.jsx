import { Box, Button, Center, Flex, Grid, Heading } from "@chakra-ui/react";
import { shallowEqual, useDispatch, useSelector } from "react-redux";
import { EmptyList } from "./EmptyList";
import { ProductBox } from "./MiniComponents";
import { useState, useEffect } from 'react';
import { RiFullscreenFill, RiFullscreenExitLine } from "react-icons/ri";
import { Loading } from "./Loading";
import { Error } from "./Error";
import { getFavouriteRequest } from "../redux/favouriteProducts/actions";

export const Favourite = () => {

    const [screen, setScreen] = useState(true);
    const dispatch = useDispatch();
    const { isLoading, favourite, isError } = useSelector((state) => state.favouriteReducer, shallowEqual);
    const token = useSelector((state) => state.authReducer.token);

    useEffect(() => {
        dispatch(getFavouriteRequest(token));
    }, [dispatch, token]);

    return isLoading ? (
        <Loading />
    ) : isError ? (
        <Error />
    ) : (
        <>
            <Flex justify={'space-between'} maxW={1200} m={'20px auto'} px={'20px'}>
                <Center>
                    <Heading>Favourite</Heading>
                </Center>
                <Center>
                    <Button
                        onClick={() => { setScreen(!screen) }}
                        leftIcon={screen ? <RiFullscreenFill /> : <RiFullscreenExitLine />}
                        display={['none', 'none', 'none', 'inline-block']}
                    >View</Button>
                </Center>
            </Flex>

            <Box>
                {favourite.length === 0 ? <EmptyList /> : <Grid className="expand" templateColumns={['repeat(1, 1fr)', 'repeat(2, 1fr)', `repeat(3, 1fr)`]} gap={'20px'} p={'20px'} maxW={screen ? 1200 : '98%'} m={'40px auto'}>
                    {favourite.map((e, i) => (
                        <ProductBox key={i} data={e} />
                    ))}
                </Grid>}
            </Box>
        </>
    );
};